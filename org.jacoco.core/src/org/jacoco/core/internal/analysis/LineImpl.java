...
public abstract class LineImpl implements ILine {
				...
				for (int k = 0; k <= SINGLETON_INS_LIMIT; k++) {
					...
					for (int l = 0; l <= SINGLETON_BRA_LIMIT; l++) {
						...
						for (int m = 0; m <= SINGLETON_BRA_LIMIT; m++) {
							SINGLETONS[i][j][k][l][m] = new Fix(i, j,
									BigInteger.valueOf(k), l, m);
						}
					}
				}
			}
	...
	private static LineImpl getInstance(final CounterImpl instructions,
			final BigInteger ec, final CounterImpl branches) {
		...
		if (im <= SINGLETON_INS_LIMIT && ic <= SINGLETON_INS_LIMIT
				...
				&& ec.compareTo(BigInteger.valueOf(SINGLETON_INS_LIMIT)) < 0) {
			return SINGLETONS[im][ic][ec.intValue()][bm][bc];
		}
	...
	private static final class Var extends LineImpl {
		Var(final CounterImpl instructions, final BigInteger executions,
		...
		public LineImpl increment(final ICounter instructions,
			...
			this.executions = executions.max(this.executions);
	...
	private static final class Fix extends LineImpl {
		public Fix(final int im, final int ic, final BigInteger ec,
		...
		public LineImpl increment(final ICounter instructions,
			...
			return getInstance(this.instructions.increment(instructions),
					this.executions.max(executions),
	...
	/** execution count */
	protected BigInteger executions;

	/** branch counter */
	protected CounterImpl branches;

	private LineImpl(final CounterImpl instructions,
			final BigInteger executions, final CounterImpl branches) {
	...
	public boolean equals(final Object obj) {
		if (obj instanceof ILine) {
			...
			return this.instructions.equals(that.getInstructionCounter())
					&& this.executions.equals(that.getExecutionCount())
